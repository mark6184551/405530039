...
$id=$_SESSION['id'];
$data=mysqli_query($conn,"select * from message order by time");
$data2=mysqli_query($conn,"select id from message order by time");
/*$user=mysqli_query($conn,"select id from message where id='$id'");*/
$data3=mysqli_query($conn,"select * from R_message order by R_time");
...
for($i=0;$i<=mysqli_num_rows($data);$i++){
  for($i=0;$i<=mysqli_num_rows($data3);$i++){
   $rs=mysqli_fetch_assoc($data);
   $rs2=mysqli_fetch_assoc($data3);
?>
<div class="container">
  <div class="CSSTableGenerator">
      <table align="center">
      <?php if($rs['id']==NULL)exit?> 
            <tr>
...
<div class="reply">
 <div class="container">
     <form id="form1" name="form1" method="post" action="">
        ...
        <?php if($rs2['R_message']!=NULL)exit?> 
    </div>
</div>
